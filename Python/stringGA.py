...
class Individual:
    def __init__(self, DNAsize):
        ...
        for i in range(DNAsize):
            self.dna.append(self.__random_letter())

    def __random_letter(self):
        abc = ' ABCDEFGHIJKLMNOPQRSTUVWXYZ'
        idx = random.randint(0, len(abc)-1)
        return abc[idx]
...
class Population:
    def __init__(self):
        self.population = []
        self.newPopulation = []
        self.goal = str(input("qual a frase obejtivo:")).upper()
    ...
    def stopcriteria(self, gen):
        ...
        print("Generation:%d\tAverage Fitness:%f\tBest Attempt:%s" %
              (gen, AvFit, phrase))
