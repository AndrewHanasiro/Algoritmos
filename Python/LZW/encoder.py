            ...
            if len(word) > ind + 1:
                for i in range(len(palavra.getProx())):
                    if palavra.prox[i].getLetra() == word[ind]:
                        palavra.addLetter(
                            palavra.prox[i], count, word, ind + 1)
...
class Dicionario():
    ...
    def addPalavra(self, word):
        self.count += 1
        self.Palavras[word[0]].addLetter(
            self.Palavras[word[0]], self.count, word, 1)
                ...
                if idx != None:  # achou no dicionario
                    ...
                    if C != -1:
                        if self.DIC.getCount() >= self.DIC.getMax():
                            ...
                            print("Total de indice no dicionário: %d" %
                                  self.DIC.getMax())
                            print("Total de indice usados: %d" %
                                  self.DIC.getCount())
                            exit(1)
                        else:
